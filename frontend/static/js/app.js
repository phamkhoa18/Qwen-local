/**
 * VKS Legal AI - Frontend v2
 * 3 Che do: RAG, Search, LLM - Fixed streaming
 */
const S = {
...
async function login() {
  ...
  try {
    ...
    if(!r.ok) throw new Error(d.detail?.error?.message||'Sai tài khoản hoặc mật khẩu');
...
async function autoKey(){
  try{
    const r=await fetch('/admin/api-keys',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${S.token}`},body:JSON.stringify({name:'Khóa mặc định',description:'Tự động tạo'})});
    const d=await r.json();
    if(d.key){S.apiKey=d.key;localStorage.setItem('vks_api_key',d.key);}
  }catch(e){}
}
function logout(){localStorage.removeItem('vks_token');localStorage.removeItem('vks_api_key');location.reload();}

// ===== INIT =====
async function init(){checkHealth();loadModels();renderConvs();setInterval(checkHealth,30000);}
...
async function loadModels(){
  ...
  if(sel&&d.data?.length)sel.innerHTML=d.data.map(m=>`<option value="${m.id}">${m.id}</option>`).join('');}catch(e){}
}
...
function setMode(mode){
  ...
  const h={rag:'Chế độ: Tra cứu & Hỏi đáp — Kết hợp cơ sở dữ liệu pháp luật + AI',search:'Chế độ: Tìm kiếm văn bản — Tìm trực tiếp trong cơ sở dữ liệu pháp luật',llm:'Chế độ: Hỏi AI trực tiếp — Trò chuyện với AI không qua cơ sở dữ liệu'};
  const p={rag:'Nhập câu hỏi pháp luật... (Enter để gửi)',search:'Nhập từ khóa hoặc nội dung cần tìm kiếm...',llm:'Nhập câu hỏi cho AI... (không sử dụng dữ liệu pháp luật)'};
...
function renderConvs(){
  ...
  if(!S.convs.length){el.innerHTML='<div style="padding:24px;text-align:center;color:var(--text-muted);font-size:12px;">Chưa có cuộc trò chuyện nào</div>';return;}
...
async function doSearch(query){
  ...
  else S.msgs.push({role:'assistant',content:'Không tìm thấy văn bản pháp luật phù hợp. Hãy thử từ khóa khác hoặc kiểm tra cơ sở dữ liệu đã được nạp chưa.',mode:'search'});
  renderMsgs();
}
...
async function startIdx(){
  ...
  await fetch(`/admin/rag/index?max_docs=${n}`,{method:'POST',headers:{'Authorization':`Bearer ${S.token}`}});
...
async function addDoc(){
  ...
  const r=await fetch('/admin/rag/add-document',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${S.token}`},body:JSON.stringify({title,content,doc_type:'legal'})});
  const d=await r.json();
  alert(`Đã thêm ${d.chunks} đoạn vào cơ sở dữ liệu`);
  document.getElementById('doc-title').value='';document.getElementById('doc-content').value='';checkHealth();
}
